...
public class BoardManager : MonoBehaviour
{
    ...
    public static BoardManager Instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                if (_instance == null)
                {
                    Debug.LogError("Fatal Error: BoardManagaer not Found");
                }
            }
            return _instance;
        }
    ...
    [Header("Tile")]
    public List<Sprite> tileType = new List<Sprite>();
    ...
    private void CreateBoard(Vector2 tileSize)
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                newTile.ChangeId(Random.Range(0, tileType.Count), x, y);
            }
        }
    }
    ...
    private List<int> GetStartingPossibleIdList(int x, int y)
    {
        ...
        if(y>1 && tiles[x, y-1].id == tiles[x, y-1].id)
        {
    ...
    {      
        ...
        List<TileController> matchingTile = GetAllMatches();
        ...
        StartCoroutine(ClearMatches(matchingTile, ProcessDrop));

        combo++;
        ScoreManager.Instance.IncrementCurrentScore(matchingTile.Count, combo);
    }

    private IEnumerator ClearMatches(List<TileController> matchingTiles, System.Action onCompleteed)
    {
    ...
    public IEnumerator SwapTilePosition(TileController a, TileController b, System.Action onCompleted)
    {
        ...
        tiles[indexA.x, indexA.y] = a;
        tiles[indexB.x, indexB.y] = b;
